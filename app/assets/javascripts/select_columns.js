;(function(HocomocoDB, $, undefined){
  ...
  HocomocoDB.ui.applyColumnSelector = function() {
    document.querySelectorAll('.select-columns-btn')
      .forEach((el) => {
        ...
        el.addEventListener('shown.bs.popover', function () { // bootstrap popover event triggered when the popover opens
          ...
          $('body').on('click', function(e) {
            if (! $(e.target).closest('#column-selector-target').length ) { // click outside
              document.querySelectorAll('.select-columns-btn').forEach((el) => bootstrap.Popover.getOrCreateInstance(el, 'hide'));
            }
          });
        });

        el.addEventListener('hidden.bs.popover', function () { // bootstrap popover event triggered when the popover opens
          $('body').off('click');
        });
      })
